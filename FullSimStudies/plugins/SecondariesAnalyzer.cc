...
#include "VFPix/FullSimStudies/plugins/SecondariesAnalyzer.h"

SecondariesAnalyzer::SecondariesAnalyzer (const edm::ParameterSet &cfg) :
  ...
  trackingVertices_ (cfg.getParameter<edm::InputTag> ("trackingVertices")),
  tracks_ (cfg.getParameter<edm::InputTag> ("tracks")),
  genParticles_ (cfg.getParameter<edm::InputTag> ("genParticles")),
  trackingParticles_ (cfg.getParameter<edm::InputTag> ("trackingParticles")),
  ...
  pfCandidates_ (cfg.getParameter<edm::InputTag> ("pfCandidates"))
{
  ...
  trackingVerticesToken_ = consumes<std::vector<TrackingVertex> > (trackingVertices_);
  tracksToken_ = consumes<std::vector<reco::Track> > (tracks_);
  genParticlesToken_ = consumes<std::vector<reco::GenParticle> > (genParticles_);
  trackingParticlesToken_ = consumes<std::vector<TrackingParticle> > (trackingParticles_);
  ...
  TFileDirectory jetDir = fs_->mkdir ("jets"),
                 ...
                 trackingParticleDir = fs_->mkdir ("trackingParticles"),
  ...
  twoDHists_["TVrhoPhiBPix"] = trackingParticleDir.make<TH2D> ("TVrhoPhiBPix", ";x [cm];y [cm]", 1000, -25.0, 25.0, 1000, -25.0, 25.0);
  twoDHists_["TVrhoZTracker"] = trackingParticleDir.make<TH2D> ("TVrhoZTracker", ";z [cm];#rho [cm]", 2000, -300.0, 300.0, 500, 0, 150.0);
  twoDHists_["TVrhoZPixels"] = trackingParticleDir.make<TH2D> ("TVrhoZPixels", ";z [cm];#rho [cm]", 2000, -300.0, 300.0, 500, 0, 22.0);

  twoDHists_["TPrhoPhi"] = trackingParticleDir.make<TH2D> ("TPrhoPhi", ";x [cm];y [cm]", 1000, -150.0, 150.0, 1000, -150.0, 150.0);
  twoDHists_["TPrhoZ"] = trackingParticleDir.make<TH2D> ("TPrhoZ", ";z [cm];#rho [cm]", 2000, -300.0, 300.0, 500, 0, 150.0);
...
SecondariesAnalyzer::~SecondariesAnalyzer ()
{
}

void
SecondariesAnalyzer::analyze (const edm::Event &event, const edm::EventSetup &setup)
{
  ...
  edm::Handle<vector<TrackingVertex> > trackingVertices;
  event.getByToken (trackingVerticesToken_, trackingVertices);
  edm::Handle<vector<reco::Track> > tracks;
  event.getByToken (tracksToken_, tracks);
  edm::Handle<vector<reco::GenParticle> > genParticles;
  event.getByToken (genParticlesToken_, genParticles);
  edm::Handle<vector<TrackingParticle> > trackingParticles;
  event.getByToken (trackingParticlesToken_, trackingParticles);
  ...
  //!!!
  for (const auto &trackingVertex : *trackingVertices)
    {

      if(trackingVertex.nSourceTracks() != 1 || trackingVertex.nDaughterTracks() != 2)
      	continue;
      if(trackingVertex.sourceTracks().at(0)->pdgId() != 22)
      	continue;

      if(fabs(trackingVertex.daughterTracks().at(0)->pdgId()) != 11)
      	continue;
      if(fabs(trackingVertex.daughterTracks().at(1)->pdgId()) != 11)
      	continue;

      // cout << "----------------\n";
      // cout << trackingVertex.nSourceTracks() << " sources\n";
      // cout << trackingVertex.nDaughterTracks() << " daughters\n";
      // cout << "sources: ";
      // for (auto &particle : trackingVertex.sourceTracks())
      // 	cout << (*particle).pdgId() << " (" << (*particle).status() << "), ";
      // cout << endl;
      
      // cout << "daughters: ";
      // for (auto &particle : trackingVertex.daughterTracks())
      // 	cout << (*particle).pdgId() << " (" << (*particle).status() << "), ";
      // cout << endl;


      double x = trackingVertex.position().x();
      double y = trackingVertex.position().y();
      double z = trackingVertex.position().z();
      if(fabs(z) < 25 && hypot (x, y) < 25)
	twoDHists_.at ("TVrhoPhiBPix")->Fill (x, y);
      twoDHists_.at ("TVrhoZTracker")->Fill (z, hypot (x, y));
      twoDHists_.at ("TVrhoZPixels")->Fill (z, hypot (x, y));


    }
  // for (const auto &trackingParticle : *trackingParticles)
  //   {
  //     if(fabs(trackingParticle.pdgId()) != 11)
  // 	 continue;
  //     double x = trackingParticle.vx();
  //     double y = trackingParticle.vy();
  //     double z = trackingParticle.vz();
  //     if(fabs(z) < 25)
  // 	twoDHists_.at ("TPrhoPhi")->Fill (x, y);
  //     twoDHists_.at ("TPrhoZ")->Fill (z, hypot (x, y));
  //   }
...
void
SecondariesAnalyzer::logSpace (const unsigned n, const double a, const double b, vector<double> &bins) const
{
...
void
SecondariesAnalyzer::linSpace (const unsigned n, const double a, const double b, vector<double> &bins) const
{
  double step = (b - a) / ((double) n);
...
bool
SecondariesAnalyzer::isMatched (const reco::Track &track, const edm::Handle<vector<reco::GenParticle> > &genParticles, const unsigned id, const double maxDeltaR, const reco::GenParticle *&closest) const
{
...
bool
SecondariesAnalyzer::isMatched (const reco::Track &track, const edm::Handle<vector<SimTrack> > &simTracks, const double maxDeltaR, const SimTrack *&closest) const
{
  double minDeltaR = 999.9;
  closest = NULL;
...
bool
SecondariesAnalyzer::isMatched (const reco::Track &track, const edm::Handle<vector<SimTrack> > &simTracks, const double maxDeltaR) const
{
  for (const auto &simTrack : *simTracks)
    {
      double dR = deltaR (track.eta (), track.phi (), simTrack.momentum ().Eta (), simTrack.momentum ().Phi ());
...
SecondariesAnalyzer::beta (const T &jet, const edm::Handle<vector<reco::Track> > &tracks, const edm::Handle<vector<reco::Vertex> > &vertices, double &sumptch, double &sumptchpv, unsigned vertexIndex) const
{
...
template<class T> double
SecondariesAnalyzer::betaStar (const T &jet, const edm::Handle<vector<reco::Track> > &tracks, const edm::Handle<vector<reco::Vertex> > &vertices, double &sumptch, double &sumptchpu, unsigned vertexIndex) const
{
...
template<class T> double
SecondariesAnalyzer::beta_dz (const T &jet, const edm::Handle<vector<reco::Track> > &tracks, const edm::Handle<vector<reco::Vertex> > &vertices, const double dzCut, double &sumptch, double &sumptchpv, unsigned vertexIndex) const
{
  sumptchpv = sumptch = 0.0;
  for (const auto &track : *tracks)
    {
...
template<class T> double
SecondariesAnalyzer::betaStar_dz (const T &jet, const edm::Handle<vector<reco::Track> > &tracks, const edm::Handle<vector<reco::Vertex> > &vertices, const double dzCut, double &sumptch, double &sumptchpu, unsigned vertexIndex) const
{
  sumptchpu = sumptch = 0.0;
  for (const auto &track : *tracks)
    {
...
template<class T> double
SecondariesAnalyzer::beta_dzSig (const T &jet, const edm::Handle<vector<reco::Track> > &tracks, const edm::Handle<vector<reco::Vertex> > &vertices, const double dzSigCut, double &sumptch, double &sumptchpv, unsigned vertexIndex) const
{
  sumptchpv = sumptch = 0.0;
  for (const auto &track : *tracks)
    {
...
template<class T> double
SecondariesAnalyzer::betaStar_dzSig (const T &jet, const edm::Handle<vector<reco::Track> > &tracks, const edm::Handle<vector<reco::Vertex> > &vertices, const double dzSigCut, double &sumptch, double &sumptchpu, unsigned vertexIndex) const
{
  sumptchpu = sumptch = 0.0;
  for (const auto &track : *tracks)
    {
...
SecondariesAnalyzer::fillTrackHistograms (const T &jet, const edm::Handle<vector<reco::Track> > &tracks, const reco::Vertex &pv) const
{
  for (const auto &track : *tracks)
    {
      if (!isGoodTrack (track))
...
double
SecondariesAnalyzer::genSumPt2 (const vector<reco::GenParticle> &genParticles) const
{
  double sumPt2 = 0.0;
...
bool
SecondariesAnalyzer::isGoodTrack (const reco::Track &track, const bool checkPt) const
{
...
bool
SecondariesAnalyzer::isMatchedToTrack (const reco::GenParticle &genParticle, const vector<reco::Track> &tracks, const double maxDeltaR, unordered_set<int> &usedTracks) const
{
...
bool
SecondariesAnalyzer::isMatchedToParticle (const reco::Track &track, const vector<reco::GenParticle> &particles, const double maxDeltaR, unordered_set<int> &usedParticles) const
{
  double minDeltaR = -1.0;
  int minDeltaRIndex = -1, i = 0;
...
bool
SecondariesAnalyzer::isMatchedToPFChargedHadron (const reco::GenParticle &genParticle, const vector<reco::PFCandidate> &pfCandidates, const double maxDeltaR, bool &isHF) const
{
...
DEFINE_FWK_MODULE(SecondariesAnalyzer);
